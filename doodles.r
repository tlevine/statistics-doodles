...
interjection('If covariance is very positive\nand x is high,\ny should be high.\n\n(We want b1 to be positive.)')
interjection('If covariance is very negative\nand x is high,\ny should be low.\n\n(We want b1 to be negative.)')
...
rect(xleft = -a, xright = 0, ybottom = a * (1 - .adj), ytop = a, col = 'blue', lty = 'blank')
text(-a/2,a/2,'cov(Petal.Width, Petal.Length)', col = 'white', font = 2)
text(0,a/2,'b1 * sd(Petal.Width)', col = 'blue', pos = 4)
...
slides <- function() {
  ...
  interjection('')
  interjection('Let\'s go over everything\none last time.')
  linear.relationships()
  covariance()
  covariance.notpositive()
  variance()
